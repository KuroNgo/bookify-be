...
type Database struct {
	...
	ClientServer   string `mapstructure:"CLIENT_SERVER"`
	AppServerHost  string `mapstructure:"APP_SERVER_HOST"`
...
func NewEnv() *Database {
	...
	if env.AppEnv == "development" {
		...
		docs.SwaggerInfo.Host = os.Getenv("APP_SERVER_HOST")
		ginSwagger.URL(fmt.Sprintf("http://%s/swagger/doc.json", env.AppServerHost))
		gin.SetMode(gin.ReleaseMode)
		log.Println("The App is running in production .env")
		fmt.Printf("Location Server Web of us: %s", env.AppServerHost)
	}
	return &env
}
