...
func AdminEventPartnerRouter(env *config.Database, timeout time.Duration, db *mongo.Database, group *gin.RouterGroup) {
	...
	router.POST("/create", eventPartner.CreateOne)
	router.PUT("/update", eventPartner.UpdateOne)
	router.POST("/delete", eventPartner.DeleteOne)
}
